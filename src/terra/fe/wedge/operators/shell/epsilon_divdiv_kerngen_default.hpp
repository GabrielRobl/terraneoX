...
class EpsilonDivDivKerngen
{
  ...
  public:
    ...
    , recv_buffers_( domain )
    {
        ...
        util::logroot << "[EpsilonDivDiv] (threads_per_column, block_size_, blocks_per_column_) = " << threads_per_column << ", " << block_size_ << ", " << blocks_per_column_ << ")" << std::endl;
    }
    ...
    void apply_impl( const SrcVectorType& src, DstVectorType& dst )
    {
        ...
        Kokkos::parallel_for( "matvec", policy, *this );
            //grid::shell::local_domain_md_range_policy_cells( domain_ ), *this );
        Kokkos::fence();
        timer_kernel.stop();
   ...
   //     operator()( const int local_subdomain_id, const int x_cell, const int y_cell, const int r_cell ) const
    ...
    KOKKOS_INLINE_FUNCTION void operator()( const Team& team ) const
    {
        ...
        else
        {
            ...
            int cmb_shift = ( at_boundary && diagonal_ == 0 && at_cmb != 0 && treat_boundary ) ? ( 3 ) : ( 0 );
            int max_rad   = radii_.extent( 1 ) - 1;
            int surface_shift = ( at_boundary && diagonal_ == 0 && at_surface != 0 && treat_boundary ) ? ( 3 ) : ( 0 );
      ...
      the components of the Epsilon operators and in the local DoFs. */
                    ...
                    {
                        if ( diagonal_ || treat_boundary && ( at_cmb != 0 || at_surface != 0 ) )
                        {
                            int node_idx;
